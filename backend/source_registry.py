...
def _codex_quota():
    ...
    from config import load_config
    cfg = load_config()
    return fetch_codex_quota(codex_bin=cfg.codex_bin, timeout=cfg.network_timeout)
...
def _normalize_codex(raw):
    """Delegates to the single canonical normalizer in codex_quota so this
    fallback registry and the YAML provider cannot drift apart."""
    from codex_quota import normalize_quota
    result = normalize_quota(raw)
    if result is None:
        return None
    # Legacy compatibility: snapshots recorded while the collector still read
    # the OpenAI billing API stored a cost figure rather than a rate limit.
    # The collector no longer produces this, but stored history may.
