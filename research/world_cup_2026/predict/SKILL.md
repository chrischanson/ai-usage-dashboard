...
8. **Clinical Finishing Efficiency (MANDATORY CHECK)**: Before predicting a team to win through volume shooting, check their goals-vs-shots and goals-vs-xG ratios across their prior tournament matches. High shot volume with low actual goals (e.g., 30+ shots, 0 goals) indicates a systemic finishing deficiency, not temporary bad luck. Do NOT assume a clinical breakthrough is coming; downgrade the team's expected goal output accordingly and raise draw/upset probability.
