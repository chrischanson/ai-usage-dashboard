...
18. **Dead Rubber Motivation Asymmetry Rule (New 2026-06-24)**: In dead rubber matches where one team's advancement is secure and the opponent faces elimination (or vice versa), the must-win team receives a motivation bonus of ~15-20% to their implied performance. If the draw-sufficient team also benches key starters (signaling conservatism through lineup rotation), the bonus may fully close the quality gap. When both conditions apply (motivation delta + starter benching), apply a one-notch confidence downgrade to the draw-sufficient team and explicitly document the motivation delta in reasoning. Validated by RSA-KOR 2026-06-24: Korea (draw-sufficient, Son benched) lost 1-0 to a motivated, must-win South Africa despite 68% possession and higher FIFA ranking.
